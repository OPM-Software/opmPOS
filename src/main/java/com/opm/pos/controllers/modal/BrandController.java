...
public class BrandController implements Initializable {
	...
	private BrandController controller= this;
    ...
    private void fetColumnList() {       
    	      ...
    	      Callback<TableColumn<Catalog, String>, TableCell<Catalog, String>> cellFoctory = (TableColumn<Catalog, String> param) -> {
    ...
    public void fetRowList() {
    	try {
    		List<Catalog> catalog = catalogService.getBrands();
    		data = FXCollections.observableArrayList(catalog);		
            tblData.setItems(data);    
            
            FilteredList<Catalog> filteredData = new FilteredList<>(data, p -> true);
    		txtFilter.textProperty().addListener((observable, oldValue, newValue) -> {
    			filteredData.setPredicate(catalogFilter -> {
    				if (newValue == null || newValue.isEmpty()) {
    					return true;
    				}
    				
    				String lowerCaseFilter = newValue.toLowerCase();				
    				if (catalogFilter.getDescription().toLowerCase().contains(lowerCaseFilter)) {
    					return true; 
    				}
    				return false; 
    			});
    ...
    private void loadModal(String opcion, Event event, Catalog cat ) {
        try {
        	...
        	CatalogController modal= loader.getController();
           	modal.initData(cat,opcion,"BRAND");        		
        	modal.setBrandController(controller);
